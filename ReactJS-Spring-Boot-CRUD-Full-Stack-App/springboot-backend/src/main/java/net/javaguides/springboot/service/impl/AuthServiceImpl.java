...
public class AuthServiceImpl implements AuthService {
    ...
    /**
     * Self-registration always creates an EMPLOYEE account. Clients cannot
     * request ADMIN or HR through this endpoint - those roles are granted
     * by an existing ADMIN (see UserController, Phase 11) or via the
     * startup bootstrap seeder for the very first admin account.
     */
    @Override
    public AuthResponse register(RegisterRequest request) {
        if (userRepository.existsByEmail(request.getEmail())) {
            throw new DuplicateUserException("An account with email '" + request.getEmail() + "' already exists");
        }
